...
def main(train_file,test_file):
  ...
  # COMMENT THIS OUT IF YOU DON'T WANT TO SAVE TEST VALUES
  test_with_bias = dummyAugment(X_test)
  y_pred_test = test_with_bias @ w_SGD >= 0
  test_out = np.concatenate((np.expand_dims(np.array(range(233),dtype=int), axis=1), y_pred_test), axis=1)
  header = np.array([["id", "cancer"]])
  test_out = np.concatenate((header, test_out))
  np.savetxt('test_cancer_predicted.csv', test_out, fmt='%s', delimiter=',')
