...
def main():
  ...
  for n in range(bagging_number):
    networks.append(FeedForwardNeuralNetwork(X_train.shape[1],10,width_of_layers,number_of_layers, activation=activation))
    ...
    # Create bootstrap dataset for each net
    n_train = np.random.choice(X_train, size=len(X_train))
    assert(n_train.shape == X_train.shape)
    ...
    for i in range(max_epochs):
      ...
      while j < len(X_train):
        ...
        logits = networks[n].forward(X_batch)
        ...
        networks[n].backward(loss_grad)

        # Take a step of gradient descent
        networks[n].step(step_size)
